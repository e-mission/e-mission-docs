...
1. Assuming that is true, [reset the pipeline](#resetting-the-pipeline)   
...
If you encounter errors like `AttributeError: property location is read-only` during pipeline execution, this means the analysis code is trying to modify wrapper data using property access instead of dictionary access.

See the [Working with Read-Only Wrappers](data_model.md#working-with-read-only-wrappers) section in the data model documentation for detailed explanations and solutions.

Quick fix: Replace `obj.property = value` with `obj["property"] = value` in your analysis code.
